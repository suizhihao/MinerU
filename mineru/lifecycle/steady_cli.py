...
def _default_entry() -> Optional[str]:
    return os.getenv("MINERU_LIFECYCLE_STEADY_ENTRY")


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run an external runtime steady-state / adaptation script (user-supplied). "
            "Set MINERU_LIFECYCLE_STEADY_ENTRY or pass --entry."
        )
    )
    parser.add_argument(
        "--entry",
        default=None,
        help="Path to Python script. Overrides MINERU_LIFECYCLE_STEADY_ENTRY.",
    )
    parser.add_argument(
        "--output-json",
        required=True,
...
def main():
    ...
    if not entry_str:
        raise SystemExit(
            "Missing steady entry: set MINERU_LIFECYCLE_STEADY_ENTRY or pass --entry /path/to/script.py"
        )
    entry = Path(entry_str).expanduser().resolve()
    if not entry.exists():
        raise FileNotFoundError(f"Steady entry not found: {entry}")
